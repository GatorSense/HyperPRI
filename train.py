...
COMET_PROJ_NAME = 'hyperpri_single'
RANDOM_STATE = 1
USE_CUDA = True
LOAD_CKPT = False
...
plant_metadata = {
    'train': f"{rel_call_path}/Datasets/HyperPRI/phenotype1.json",
    'val': f"{rel_call_path}/Datasets/HyperPRI/phenotype2.json",
    'test': f"{rel_call_path}/Datasets/HyperPRI/phenotype2.json"
}
...
train_params = {
    ...
    "channels": 3,  # 3 for RGB, 299 for HSI, set to 1 for 3D-UNET
    "bilinear": False,
    "feature_extraction": False,
    "use_attention": False,
    '3d_featmaps': 10,          ## How many feature maps are in 3D_UNET's first layer
    ...
    'ckpt_run': 2,   # Which split to start from
    'ckpt_subdir': "UNET",
    'hist_reg': 0.0,
...
for run in range(start_run, train_params['splits']):
    ...
    if LOAD_CKPT and run != start_run:
